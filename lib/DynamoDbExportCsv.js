...
function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, sessionToken, awsRegion) {
  // Events this emits
  var infoEvent = 'info';
  ...
  if (_sessionToken) {
    config.sessionToken = _sessionToken;
    _args.push('--sessiontoken');
    _args.push(_sessionToken);
  }
  ...
  function writeTableToCsv(table, columns, totalSegments, segment, compressed, filesize, s3Bucket, s3Path, callback) {
    ...
    async.doWhilst(
      function (done) {
        ...
        // then th segemnt and the current date
        var fileName = table + "-" + segment + "-" + fileCount + "-" + currentDate + ".csv";
        if (compressed) {
          fileName += ".gz";
        }
